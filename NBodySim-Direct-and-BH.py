# Title: Direct N-Body and Barnes-Hut Approximation Simulation
# Authors: Benjamin Smith with some assistance from Braeden Farley
# Date: 12/6/24
...
#   setuptools  75.6.0
# Description:
#   This program is a 2D N-Body Simulation that allows the user to simulate celestial bodies using both
#   the direct n-body algorithm and the Barnes-hut approximation. There are utilities to add and remove planets,
#   render overlays to illustrate both algorithms, and output runtime differences mean error between the two
#   algorithms.
...
class GraphWindow(pyglet.window.Window):
    ...
    def update(self, dt:float): #physics update. run rate controlled in main
        if not self.is_paused:
            ...
            NBodySim.update_bodies(self, self.body_list)  # update all bodies according to the direct-n-body simulation approach
                ...
                for body in self.body_list:
                    for body2 in self.body_list:
                        self.graph_line_list.append(
                            shapes.Line(body.square.x, body.square.y, body2.square.x, body2.square.y, 1, color=(64, 64, 64), batch=self.main_batch))       
    ...
    def on_key_press(self, symbol: int, modifiers: int) -> None:
        if symbol == key.N: # Start a simulation with one stationary body
            ...
            self.body_list.extend(random_body_list(self, 20, 50, 2, 2))
            print(f"Bodies: {len(self.body_list)}")
        elif symbol == key._2: # add 1 heavy body
            self.body_list.extend(random_body_list(self, 300, 50, 2, 2))
            print(f"Bodies: {len(self.body_list)}")
        ...
        elif symbol == key.H: # toggle between direct sim and Barnes Hut
        ...
        # testing accuracy -  output code written by Braeden Farley
        elif symbol == key.T:
            print_to_txt = True
...
def random_body_list(gwindow: GraphWindow, num_bodies: int, mass: float, width: float, height: float): #creates a list of random bodies
    body_list = []
    for i in range(num_bodies):
        body_list.append(CBody(shapes.Rectangle(0, 0, width, height, batch=gwindow.main_batch),
                               random.uniform(mass,mass), [random.uniform(1,gwindow.width),
                                                random.uniform(1,gwindow.height)], [random.uniform(0,0),random.uniform(0,0)]))
    return body_list

def random_vortex_list(gwindow: GraphWindow, num_bodies: int, mass: float, width: float, height: float): #creates a list of random bodies
    body_list = []
    ...
    for i in range(num_bodies):
        body_list.append(CBody(shapes.Rectangle(0, 0, width, height, batch=gwindow.main_batch),
                               random.uniform(mass,mass), (x,y), NBodySim.calculate_centripetal_velocity(x, y, gwindow.width/2, gwindow.height/2, 0.5)))
    return body_list
...
def find_bounding_square(bodies: [], margin=0): #returns the corner and side length of a square that encompasses all the bodies.
    if not bodies:
        return 0.0, 0.0, 1.0
    ...
    for i in range(4):  # for each quadrant
        if len(quadrants[i]) > 0:  # if quadrant has bodies
            ...
            if i == 0:
                ...
                root.children[i] = BHTree(root, (root.cornerx + root.width/2), root.cornery, root.width / 2)  # top right
            elif i == 2:
                root.children[i] = BHTree(root, root.cornerx, (root.cornery + root.width/2), root.width / 2)  # bottom left
            elif i == 3:
                root.children[i] = BHTree(root, (root.cornerx + root.width/2), (root.cornery + root.width/2), root.width/2)  # bottom right
...
class CBody:
    ...
    #getter and setter written by Braeden Farley
...
class NBodySim:
    ...
    # bh_force method written co-operatively by both me and Braeden Farley
    @staticmethod
...
if __name__ == '__main__':
    window = GraphWindow(width=1280, height=720, caption="Planetary System")
    pyglet.clock.schedule_interval(window.update, 1 / 1000)
